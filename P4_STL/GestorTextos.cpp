...
bool GestorTextos::buscarTermino(std::string termino, Palabra result){
    ...
    result=_diccionario.buscarTermino(termino).getPalabra();
    if(_diccionario.buscarTermino(termino).getPalabra()!=""){
        encontrado=true;
    }
    return encontrado;
}
std::list<Palabra> GestorTextos::buscarFamilias(std::string raiz){
    return _diccionario.buscarFamilias(raiz);
}
